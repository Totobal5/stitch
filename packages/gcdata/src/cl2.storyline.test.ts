...
import { parseStringifiedStoryline, stringifyStoryline } from './cl2.storyline.js';
...
describe('Cl2 Storylines', function () {
  it('can convert a storyline mote to a text format', async function () {
    ...
    await pathy(`tmp.${storylineSchemaId}`).write(stringifyStoryline(storylines[0], packed));
  });
  ...
  it('can convert storylines to text and back without error', async function () {
    ...
    for (const storyline of storylines) {
      ...
      if (results.diagnostics.length > 0) {
        console.error('Storyline not parsed:', storyline.id, storyline.data.name);
