...
export async function fetchXml<T = unknown>(url: string, validator?: Validator<T>): Promise<T> {
...
export async function fetchJson<T = unknown>(url: string, validator?: Validator<T>): Promise<T> {
  const res = await fetchUrl(url);
  try {
